...
if (uname | get kernel-name) == 'Darwin' and (uname | get machine) == 'arm64' {
...
if (which delta | is-not-empty) {
	$env.DELTA_FEATURES = 'line-numbers hyperlinks' # Features to enable
	(which moor | is-not-empty); $env.DELTA_PAGER = 'moor --no-linenumbers --quit-if-one-screen --no-clear-on-exit' # Pager
}
...
(which opencode | is-not-empty); $env.OPENCODE_EXPERIMENTAL = 'true'                                 # Enable experimental features # TODO: Remove this
